...
const DateRangeInput = (props) => {
    const { input, label = false, required, meta: { error, touched }, placeholder } = props;
    ...
    return (
        <div ref={setWrapperRef} className={`${style.formGroup}${touched && error ? (" " + style.hasError) : ''}`}>
            {
                label !== false && (
                    <Label text={label} required={required} />
                )
            }
            <div className={`${style.inputWrap} ${style.dateRangeInput}`} onClick={toggleDatePicker}>
                {(sd && ed) ? <div className={`${style.textbox}`}>{sd} - {ed}</div> : <div className={`${style.textbox} ${style.placeholder}`}>{placeholder}</div>}
                <i className={`${style.icon} ${style.calendar} ${style.inputIcon}`} />
            </div>
            {picker &&
                <div className={`${style.floatingDateRange}`}>
                    <Button label='Clear' classes={`${style.active} ${style.dateRageClear}`}
                        onClick={() => {
                            input.onChange("");
                    ...
                    />
                    <Button label='Done' classes={`${style.active} ${style.dateRageDone}`}
                        onClick={() => {
                            input.onChange({
