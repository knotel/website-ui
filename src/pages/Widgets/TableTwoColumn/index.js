                        ...
                        {items.length >= 2 && <div className={`section_border_middle_2`}></div>}
                        {
                            chunks.map((items, cIndex) => (
                                items.map((item, index) => {
                                    ...
                                    return (
                                        <div className={`section_item ${style.item}`} key={`text_table_${index}`}>
                                            {!islastRow && (index % 2 === 0) && <div className={`section_item_border_bottom`}></div>}
                                            <div className={style.title}>
                                                <EqualHeightElement name="TextTableTitle">{item.title}</EqualHeightElement>
                                            </div>
                                            <EqualHeightElement name="TextTableText">
                                                <LazyLoad>
                                                    <div className={style.table_text}>
                                                        <div className={style.text} dangerouslySetInnerHTML={{ __html: item.text }}></div>
                                                    </div>
                                                </LazyLoad>
                                            </EqualHeightElement>
                                        </div>
